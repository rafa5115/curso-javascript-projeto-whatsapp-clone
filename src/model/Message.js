...
export class Message extends Model {
    ...
    get lastMessage() { return this._data.lastMessage; }
    set lastMessage(value) { this._data.lastMessage = value; }

    get lastMessageTime() { return this._data.lastMessageTime; }
    set lastMessageTime(value) { this._data.lastMessageTime = value; }

    static getRef(chatId) {
        return Firebase.db().collection('chats').doc(chatId).collection('message');
    }
    ...
    static sendAudio(chatId, user, file, metadata) {
        Message.send(chatId, user.email, 'audio', '').then(msgRef => {
            Message.upload(file, user.email).then( snapshot => {
                snapshot.ref.getDownloadURL().then(downloadURL => {
                    let downloadFile = downloadURL;
                    msgRef.set({
                        content: downloadFile,
    ...
    getViewElement(me = true) {
        ...
        messageEl.id = `_${this.id}`
        //messageEl.dataset.email = 
        ...
        switch (this.type) {
            ...
            if (this.content.photo) {
                let photoEl = messageEl.querySelector('.photo-contact-sended');
                photoEl.src = this.content.photo;
                photoEl.show();
            }
            ...
            messageEl.querySelector('.message-photo').on('load', e => {
                ...
                messageEl.querySelector('._3v3PK').css({ height: 'auto' });
            })
            ...
            if (this.photo) {
                let photoEl = messageEl.querySelector('.message-photo');
                photoEl.src = this.photo;
                photoEl.show();
            }

            let audioEl = messageEl.querySelector('audio');
            let audioLoadEl = messageEl.querySelector('.audio-load');
            let btnPlayEl = messageEl.querySelector('.audio-play');
            let btnPauseEl = messageEl.querySelector('.audio-pause');
            let inputRangeEl = messageEl.querySelector('[type=range]');
            let audioDurationEl = messageEl.querySelector('.message-audio-duration');
            ...
            audioEl.onpause = e => {
                audioDurationEl.innerHTML = Format.time(this.duration * 1000);
                btnPlayEl.show();
                btnPauseEl.hide();
            }
            ...
            audioEl.ontimeupdate = e => {
                btnPlayEl.hide();
                btnPauseEl.hide();
                audioDurationEl.innerHTML = Format.time(audioEl.currentTime * 1000);
                inputRangeEl.value = (audioEl.currentTime * 100) / this.duration;
            ...
            btnPlayEl.on('click', e => {
                audioEl.play();
            })

            btnPauseEl.on('click', e => {
                audioEl.pause();
            })

            inputRangeEl.on('change', e => {
                audioEl.currentTime = (inputRangeEl.value * this.duration) / 100;
            })
        ...
        if (me) {
            ...
            messageEl.querySelector('.message-time').parentElement.appendChild(this.getStatusViewElement());
        }

        messageEl.firstElementChild.classList.add(className);
